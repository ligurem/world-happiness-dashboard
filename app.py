...
current_map_selected_countries = st.session_state.get("map_selected_countries", []) or []
if map_selected_countries_from_click != current_map_selected_countries:
    removed_from_map = [
        country_key
        for country_key in current_map_selected_countries
        if country_key not in map_selected_countries_from_click
    ]
    added_from_map = [
        country_key
        for country_key in map_selected_countries_from_click
        if country_key not in current_map_selected_countries
    ]

    updated_manual_countries = [
        country_key
        for country_key in (st.session_state.get("selected_countries_manual", []) or [])
